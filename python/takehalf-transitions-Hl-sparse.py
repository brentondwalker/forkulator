...
def lh_matrix_size(s, k):
    return int((s+1) + (k-1)*s*(s+1)/2)
...
def create_diagonal_entries(Q, s, mu, lmbda):
    print(f"create_diagonal_entries(s={s}, mu={mu}, lmbda={lmbda})")
    k = s
    i = 0
    for b in range(0, s+1):
        for H in range(b, b*k+1):
            l = s - b
            phi = total_departure_probability(b, H, s)
            phi_bar = 1.0 - phi
            entry = 0
            if l > 0:
                entry -= lmbda
...
def create_sparse_transition_matrix(s:int, mu:float, lmbda:float):
    # first compute the number of states
    k = s
    numstates:int = lh_matrix_size(s, k)
    ...
    create_diagonal_entries(Q, s, mu, lmbda)
...
def main():
    ...
    Q_block = create_sparse_transition_matrix(s, mu, lmbda)
    print(Q_block)
