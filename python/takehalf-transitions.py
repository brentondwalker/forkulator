...
class SysState:
    ...
    class Transition:
        ...
        def __str__(self):
            return "Transition: "+str(self.S1.l)+" --> "+str(self.S2.l)+"\tweight= "+str(self.weight)+"\tstart="+str(self.job_start)+"\tdeparture="+str(self.task_departure)
    ...
    def __init__(self, s, l):
        ...
        # sanity check
        print("\ncreate SysState: (S, B, L) = (", self.s, self.b, self.l, ")  ID=", self.state_id)

    def __str__(self):
        return "SysState: (s,b,l)=("+str(self.s)+","+str(self.b)+","+str(self.l)+")"

    def add_transition(self, S2, weight, job_start=False, task_departure=False):
        self.transitions[S2.l] = SysState.Transition(self, S2, weight, job_start, task_departure)
...
def traverse_states(states: dict[tuple,SysState], S: SysState, lmbda, mu, take_frac = 0.5) -> object:
    ...
    while len(unvisited) > 0:
        ...
        if S1.l < S1.s:
            # to compute the transition rate, we need to compute the
        ...
        if S1.l > 0:
            ...
            #parallelism = math.ceil(S1.l * take_frac - 1.0e-5)
            parallelism = min(S1.l*take_frac, S1.s*take_frac)
            print("\t S2 parallelism = ", S1.l, "*", take_frac, " = ", (S1.l*take_frac), "=",  math.ceil(S1.l*take_frac), " = ", parallelism)
