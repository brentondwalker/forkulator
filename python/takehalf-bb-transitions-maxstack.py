...
def traverse_states_maxstack(states: dict[tuple,SysState], S: SysState, lmbda, mu, take_frac = 0.5) -> object:
    ...
    while len(unvisited) > 0:
        ...
        if S1.l > 0:
            ...
            # if the current tallest job is taller than whet we're adding, stick with that height.
            # it is rare that this will make a difference.
            # in fact, the difference is almost undetectable
            h2 = max(h1, math.ceil(S1.s / math.ceil(parallelism)))
            if (l2,h2) not in states:
                states[l2,h2] = SysState(S1.s, l2, h2)
            S2 = states.get((l2,h2))
