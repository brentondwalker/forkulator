 ...
 *
 */
public class FJRandomTaskServiceProcessPartitionJob extends FJPartitionJob {
    ...
    FJRandomTaskServiceProcessPartitionJob() {}
    /**
     * Constructor
     *
     * @param num_tasks
     ...
     * @param arrival_time
     */
    public FJRandomTaskServiceProcessPartitionJob(int num_tasks, int num_workers, IntertimeProcess service_process, IntervalPartition job_partition_process, double arrival_time) {
        this.num_tasks = num_tasks;

        tasks = new FJTask[this.num_tasks];
        this.num_tasks = num_tasks;
        this.job_service_time = 0;
        ...
        for (int i=0; i<this.num_tasks; i++) {
            if (job_partition_process.independentSamples()) {
                double job_service_time = 0.0;
                for (int j=0; j<num_workers; j++) {
                    job_service_time += service_process.nextInterval();
                }
                tasks[i] = new FJTask(job_partition_process.getNewPartition(job_service_time, this.num_tasks), arrival_time, this);
            } else {
    ...
    FJPartitionJob createNewInstance(int num_tasks, int num_workers,
        ...
        return new FJRandomTaskServiceProcessPartitionJob(num_tasks, num_workers, service_process,
